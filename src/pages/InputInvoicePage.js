...
const InputNewItem = ({ onAddNewItem }) => {
  const getDetail = async (data) => {
    ...
    setSpecs(res.data.data.map((v) => ({ ...v, id: v.id, label: v.product_types[0].name })));
  };
      ...
      >
        <div style={{ margin: 'auto', backgroundColor: 'white', padding: '14px' }}>
          <FormContainer onSuccess={(data) => onSubmit(data)}>
            ...
            />
            <SelectElement label="Loại" name="type" fullWidth required options={specs} />
            <Button type={'submit'} variant="contained" fullWidth style={{ marginTop: '14px' }}>
              Thêm
            </Button>
          </FormContainer>
        </div>
...
const InputInvoicePage = () => {
  ...
  const onSubmit = (values) => {
    const userData = {
      ...values,
      payment_method: values.payment_method === 1 ? 'Tiền mặt' : 'Chuyển khoản',
    ...
    axiosClient.post('/api/input-invoices', userData).then((res) => {
      if (res.data.message === 'Successful') {
        ...
        toast.error('Không thành công. Vui lòng thử lại!', { autoClose: 2000 });
      }
    });
  };
  ...
  return (
    <>
      ...
      <Modal
        open={openAdd}
        children={<InputNewItem onAddNewItem={onAddNewItem} />}
      ...
      <Grid container spacing={2}>
        ...
        <Grid xs={4}>
          <FormContainer onSuccess={(data) => onSubmit(data)}>
            <SelectElement label="Nhà cung cấp" name="supplier_id" fullWidth options={listSupplier} />
            <TextFieldElement name="deliver_name" label="Người giao" required fullWidth style={{ marginTop: '14px' }} />
            <br />
            <TextFieldElement
              name="deliver_phone"
            ...
            <Button type={'submit'} variant={'contained'} color={'primary'} style={{ marginTop: '14px' }}>
              Lưu
            </Button>
            <Button variant={'contained'} color={'error'} style={{ margin: '14px 0 0 14px' }}>
              Huỷ bỏ
            </Button>
          </FormContainer>
        </Grid>
